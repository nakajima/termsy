	...
	final class MacWindowBlurController {
		...
		private init() {
			self.skyLightHandle = dlopen("/System/Library/PrivateFrameworks/SkyLight.framework/SkyLight", RTLD_LAZY)

			if let skyLightHandle,
			   let symbol = dlsym(skyLightHandle, "CGSDefaultConnectionForThread")
			{
				self.defaultConnectionForThread = unsafeBitCast(symbol, to: DefaultConnectionFunction.self)
			} else {
				self.defaultConnectionForThread = nil
			}

			if let skyLightHandle,
			   let symbol = dlsym(skyLightHandle, "CGSSetWindowBackgroundBlurRadius")
			{
				self.setWindowBackgroundBlurRadius = unsafeBitCast(symbol, to: SetBlurRadiusFunction.self)
			} else {
				self.setWindowBackgroundBlurRadius = nil
			}
		}
