	...
	final class LocalShellSession {
		...
		private var shellIntegrationCleanupDirectory: URL?
		...
		func start() throws {
			...
			shellIntegrationCleanupDirectory = shellLaunch?.cleanupDirectory
			...
			if pid == 0 {
				...
				let arguments = shellLaunch?.arguments ?? ["-\(shellName)"]
		...
		private func cleanupShellIntegrationDirectory() {
			guard let shellIntegrationCleanupDirectory else { return }
			try? FileManager.default.removeItem(at: shellIntegrationCleanupDirectory)
			self.shellIntegrationCleanupDirectory = nil
		}
	}
#endif
